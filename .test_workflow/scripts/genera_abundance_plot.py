...
def create_bubble_plots_combined(
    ...
    for i, amr in enumerate(amr_gene_families, start=1):
        ...
        genus_overlap = (
            set.intersection(*top_genera_per_sample_set.values())
            if top_genera_per_sample_set
            else set()
        )
        ...
        if len(genus_overlap) >= min_overlap:
            ...
            sample_iters = {
                sample: iter(genera)
                for sample, genera in top_genera_per_sample_list.items()
            }
